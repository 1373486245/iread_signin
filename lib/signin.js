module.exports=(account_id)=>{
    return new Promise(async(resolve,reject)=>{
        ...
        while( !isBreak ){
			...
			let getGiftRes;
			if( giftTime++<8 ){
				getGiftRes=await getGift(account_id,true);
			}else{
				getGiftRes=await getGift(account_id);
			}
            if( typeof getGiftRes=='object' ){
                ret.value++;
                console.log('剩余执行次数：'+getGiftRes.daySurplus);
                if( getGiftRes.daySurplus==0 ){
                    // console.log('执行完退出');
                    isBreak=true;
                    ret.finish=true;
                }
...
function getGift(account_id,u120=false){
	var ct=u120?'&sendType=2':'';
    return new Promise((resolve,reject)=>{
        const request = require('request');
        request({
            url:'http://st.woread.com.cn/touchextenernal/readActivity/sendRightOfGoldCoin.action?userType=112_3001&homeArea=036&homeCity=468'+ct,
            method:'GET',
            headers:{
                'Cookie':`useraccount=${account_id};`,
                'Content-Type':'application/json; charset=UTF-8'
